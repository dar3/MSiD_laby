...
firstProductPrice = 25
secondProductPrice = 13
meatSupply = 1000
fillerSupply = 500
saltSupply = 250
def income(x_1: float, x_2: float) -> float:
    """Income from production of flesh."""
    return secondProductPrice * x_1 + firstProductPrice * x_2
...
def optimise() -> Tuple[float, float]:
    ...
    x0 = (0,0)
    limitations = [constr_flesh, constr_filler, constr_salt, constr_x_1, constr_x_2]
    x_opt = optimize.fmin_cobyla(
        func=objective,
        x0 = x0,
        cons = limitations,
    )
    # optimization result x1 and x2
    print(x_opt)
    return x_opt
...
if __name__ == "__main__":
    ...
    print(f"Amount of: x_1 = {round(x_1_opt, 2)} kg")
    print(f"Amount of: x_2 = {round(x_2_opt, 2)} kg")
    print(f"Max income: {round(income(x_1_opt, x_2_opt))} zł")
